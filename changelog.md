* 1.10.5 (merge 1.5.14)
  - Split batch if `message_too_large` error code is received.
    Prior to thix fix, `wolff_producer` would retry the same batch indefinitely for any error code received from Kafka (`message_too_large` included).
    After this fix, if `message_too_large` happens, `wolff_producer` splits the batch into single-message batches and retry.
